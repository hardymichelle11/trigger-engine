...
### 2026-05-03 (session 35 — Phase 4.7.6: Replay Last Close — after-hours / weekend staging mode)

Added a real after-hours / weekend staging workflow so the operator can stage candidates against the last completed market session instead of synthetic mock data. Sample mode preserved for UI demos; Replay Last Close is a separate, honest operator workflow with end-to-end indicators (REPLAY pill on the command bar, badge on each card, banner on the detail panel) so replay analysis is never confused with live data.

**Pure helpers + scanner integration**: `src/engines/discovery/freshnessPolicy.js` got a new `replay` session that returns `{ disabled: true }` for all three scanner modes — same treatment as `closed` but a distinct value so the UI can render a deliberate REPLAY indicator instead of a passive "market closed" warning. `src/components/discovery/lethalBoardScanController.js` gained two new SCAN_MODE entries (`REPLAY_LAST_CLOSE` preview-only, `COMMIT_REPLAY` records-but-tagged), a `RECORDED_SOURCE` constants export, and the commit gate now forwards `recordedSource` into the wireup. `src/engines/discovery/discoveryAlertWireup.js` `route(scanResult, { recordedSource })` stamps the source onto every persisted alert (defaulting to `"live"` for back-compat). `src/lib/alerts/alertHistory.js` persists `recordedSource` on every history entry.

**The replay data loader** (`src/engines/discovery/replayLastCloseLoader.js`, new): pure module exporting `fetchReplayLastCloseBundle({ fetcher, now, curatedSymbols, timeoutMs })` that returns a bundle in the exact shape `runMarketDiscoveryScan` already consumes. Three tiers: **Tier 1 — BQ `bars_1d`** (stub returning `null` with `// TODO(phase 4.8?)` — frontend has no BQ read path today; explicit deferral rather than a fake implementation); **Tier 2 — Polygon grouped daily** `/v2/aggs/grouped/locale/us/market/stocks/{date}` (PRIMARY — single HTTP call returns OHLCV for every US stock, probe walks back from "today" up to 5 calendar days to find a non-empty trading date so weekends + holidays Just Work without a holiday calendar); **Tier 3 — per-symbol previous-close** `/v2/aggs/ticker/{symbol}/prev` (fallback when grouped fails). Single-day pricing approximations explicitly labeled in the bundle (`metadata.replay = true`): `price` ← session close, `previousClose` ← session open (NOT yesterday's close), `atr` ← `high - low`. No `iv`, no `optionsDataBySymbol` — replay shows underlying analysis only; cards naturally surface "Option chain not verified". The loader never throws — failure path returns a structured fallback bundle with `symbols: []` and a populated `metadata.universe.reason`.

**LethalBoardPage wiring**: new `runReplayLastClose` callback alongside the existing sample / live / commit callbacks. `runLiveCommit` now branches on `liveMeta?.replay`: if a replay scan is active, a `window.confirm` dialog warns "Recording will commit this as a REPLAY alert (audit-tagged). It will not overwrite live alerts." On confirmation, the existing replay scanResult is re-applied with `SCAN_MODE.COMMIT_REPLAY` (no new HTTP fetch — the data is already loaded), which threads `recordedSource: "replay_last_close"` onto the persisted alert. Replay-recorded alerts and live-recorded alerts coexist in the audit list without ever being silently confused.

**Operator Console — 4-button Actions**: new "Replay Last Close" button between Run Scan (sample) and Live Preview, styled with an amber border + amber text on a soft amber wash to mark it as the after-hours staging lane. `aria-label="Replay last close"`.

**REPLAY indicators end-to-end**: `CapitalCommandBar` renders an amber `● Replay · {sessionDateLabel}` pill when `liveMeta?.replay === true`. `OpportunityCard` defines a new `<ReplayBadge>` primitive that leads the action row when replay is active. `OpportunityDetailPanel` renders an amber-bordered banner flush below the quote header with the line "This is replay analysis from the last completed session. Confirm live pricing before entry." `TopPicksGrid` and `LethalBoardCockpit` thread the replay flags from a single source (`liveMeta?.replay`) so the entire surface flips to REPLAY mode in one frame.

**Phase coverage**: new `scripts/test-discovery-phase4-7-6.js` registered as `npm run test:discovery-phase4-7-6` — 92 assertions across 16 groups: file existence + JSX validity, freshness `replay` session, replay loader pure helpers + grouped-daily weekend probe, per-symbol fallback, structured-fallback bundle, scan-mode + recorded-source constants, scan-controller preview-vs-commit semantics, wireup recordedSource threading, alertHistory persistence, OperatorConsole 4th button + onClick count ≥ 6, command-bar pill, ReplayBadge, detail-panel banner, LethalBoardPage wiring + commit confirm, cockpit thread-through.

**Build status at session end**: full 4.7.x matrix green — 4.7.2 (225) + 4.7.3 (162) + 4.7.4 (78) + 4.7.5 (117) + 4.7-ui (289) + 4.7.6 (92) = **963 phase assertions, 0 failures**. Older regression suites green: `test:discovery` (26/0), `test:discovery-phase4-1` clean (alertHistory recordedSource defaults preserved back-compat), `test:discovery-phase4-5a1` (72/0 — freshness policy unchanged for existing sessions), `test:alerts` (34/0). `npm run validate` 100/100 clean, `npm run build` clean. Frozen surfaces (Phase 1–4.4) untouched; Trigger Engine and CreditView unchanged. Out of scope: BQ tier (deferred), multi-day pricing context (single-day proxy is honest for v1), option chains for replay (intentional — replay shows underlying analysis, not chain analysis).

### 2026-05-03 (session 34 — Phase 4.7.5.x: comparative insight, mission-control Operator Console, OpportunityCard overlay cleanup)

Three focused, slow-step refinements to the Lethal Board cockpit. No engine changes, no theme palette changes, no layout grid changes.

**Comparative insight helper** — new pure module `src/components/discovery/cockpit/cockpitInsight.js` exporting `buildComparativeInsight(row, peerRows, tcBySymbol)`. Ten-priority decision list (top-rank+best → highest premium-per-risk → better capital fit than higher-ranked peer → only live chain → tighter spread → greater ATR cushion → faster premium decay → engine `displacedBy` → best-use → `reasonSummary` fallback). Reads only sanitized view-model + trade-context fields; references no engine internals; never throws on missing data. Wired into `TopPicksGrid.jsx` (computes against the **full** `rows` array so peers outside the top three remain referenceable) and `OpportunityCard.jsx` (renders the comparative line in preference to the raw `reasonSummary`).

**Operator Console mission-control rebuild** (`src/components/discovery/cockpit/OperatorConsole.jsx`, full rewrite) — the left rail now organizes as five blocks: **CAPITAL** (inline `$` editors for Start + Deployable that commit on blur/Enter, Hide-balances toggle, "Edit Capital → modal" button for the rest), **MODE** (4-button toggle wired directly to `marketMode`: defensive / neutral / risk_on / opportunistic, persists via the existing `useCapitalContext.saveContext`), **ACTIONS** (Run Scan primary white solid → sample, Live Preview neutral outline → real no-save, Run & Record teal accent → real with persist), **ALERTS** (compressed 4-chip rollup in 2×2 grid: 24h / 7d / new best / displaced — per-row list moved to Diagnostics), **DIAGNOSTICS** (collapsed disclosure for Polygon universe, ThetaData options, Last scan, recent recorded). ErrorBlock stays always visible at the very top when present. Canonical strings preserved as `aria-label` / `title` so phase-test substring checks pass. Threaded a new `onSaveCapital` prop through `LethalBoardCockpit.jsx` → `OperatorConsole`, sourced from `saveContext` in `LethalBoardPage.jsx` — same per-user UUID-scoped localStorage key the modal already writes to, same cross-tab `storage`-event subscription, no migration risk.

**OpportunityCard overlay cleanup** (`src/components/discovery/cockpit/OpportunityCard.jsx`) — `<ScoreRing>` moved off the chart entirely (was top-right, collided with TradingView's title bar; bottom-right interim move rejected as still cluttered) and now lives inline at the right end of the action row (`marginLeft: "auto"`, 32 px), so the engine-assessment cluster reads left-to-right as: `★ best · BUY · breakout · fit excellent · 87`. Redundant `rank #N` text removed from the chart bottom-right (the SLOT label at top-left already shows the same number). `★ BEST USE` indicator moved out of the chart top-left where it overlapped TradingView's company-name header — replaced by a new `<BestUseBadge>` primitive that renders inline as the leading element of the action row when `row.isBestUseOfCapital === true`.

**Build status at session end**: all five phase 4.7.x test scripts green (225 + 162 + 78 + 117 + 289 = 871 assertions, 0 failures), `npm run validate` 100/100 clean, `npm run build` clean. Frozen surfaces (Phase 1–4.4) untouched; Trigger Engine and CreditView unchanged. Out of scope this session: theme palette swap, lower workspace 65/35 split, OpportunityCard structural rebuild — all deferred.
